...
VALUES ('EXP_JUZGADOS', 'Juzgados en expedientes', 'mdi mdi-gavel', '/exp_juzgados', TRUE);

-- Hacemos que la tabla de `permisos` el campo `id` tipo `uuid` sea autogenerado
ALTER TABLE permisos 
ALTER COLUMN id SET DEFAULT gen_random_uuid();

-- Añadir a la tabla de `permisos` que el rol ADMINISTRADOR pueda administrar el nuevo módulo EXP_JUZGADOS
INSERT INTO permisos (rol_id, modulo_id, nombre, nivel)
SELECT 
    r.id AS rol_id, 
    m.id AS modulo_id, 
    'ADMINISTRADOR puede ADMINISTRAR en EXP_JUZGADOS' AS nombre, 
    4 AS nivel
FROM roles r, modulos m
WHERE r.nombre = 'ADMINISTRADOR' 
  AND m.nombre = 'EXP_JUZGADOS';